...
class ASFF_2(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[128,256]):
        ...
        self.upsample = Upsample(channel[0], channel[1])
...
class ASFF_3(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[512,256,128]):
        ...
        self.level = level
        if self.level == 0:
            self.upsample4x = Upsample(channel[0], channel[1], scale_factor=4)
            self.upsample2x = Upsample(channel[2], channel[1], scale_factor=2)
        elif self.level == 1:
            self.upsample2x1 = Upsample(channel[0], channel[2], scale_factor=2)
            self.downsample2x1 = Downsample(channel[1], channel[2], scale_factor=2)
        elif self.level == 2:
            self.downsample2x = Downsample(channel[2], channel[0], scale_factor=2)
            self.downsample4x = Downsample(channel[1], channel[0], scale_factor=4)

    def forward(self, x):
        input3, input1, input2 = x
